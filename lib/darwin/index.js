...
function darwinSnapshot (options = {}) {
  ...
  const displaysPromise = options.displays ? Promise.resolve(options.displays) :listDisplays();

  return displaysPromise.then((displays) => { return performScreenCapture(displays) })
}
